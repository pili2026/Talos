...
from api.model.responses import WiFiConnectionResponse, WiFiListResponse, WiFiNetwork
...
class WiFiService:
    ...
    def _check_network_manager(self) -> bool:
        ...
        """
        try:
            result = subprocess.run(["which", "nmcli"], capture_output=True, check=False, text=True, timeout=5)
    ...
    async def scan_networks(self) -> WiFiListResponse:
        ...
        try:
            ...
            subprocess.run(
                ["nmcli", "device", "wifi", "rescan"], capture_output=True, check=False, text=True, timeout=10
            )
    ...
    async def connect_to_network(self, ssid: str, password: str | None = None) -> WiFiConnectionResponse:
        ...
        try:
            ...
            if has_profile:
                ...
                result = subprocess.run(
                    ["nmcli", "connection", "up", ssid], capture_output=True, check=False, text=True, timeout=30
                )
            else:
